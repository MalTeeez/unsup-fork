...
public class NativeHandler extends AbstractFormatHandler {
    ...
    private static final class PathAndHash {
        ...
        PathAndHash(String path, String hash) {
            this.path = path;
            this.hash = hash;
        }
        ...
        if (ourVersion != null) {
            ...
            for (Object o : history) {
                if (o instanceof JsonObject jo) {
                    ...
                    if (v != null && v.code() == ourVersion.code()) {
                        alreadyTracked = true;
                        break;
                    }
                }
        ...
        if (ourVersion == null) {
            ...
            if (bootstrap != null) {
                ...
                for (Object o : bootstrap.getArray("files")) {
                    if (!(o instanceof JsonObject file))
                        throw new IOException("Entry " + o + " in files array is not an object");
                    ...
                    if (hash.length() != func.sizeInHexChars())
                        throw new IOException(path + " in files array hash " + hash + " is wrong length (" + hash.length() + " != " + func.sizeInHexChars() + ")");
                    long size = file.getLong("size", -1);
                    if (size < 0) throw new IOException(path + " in files array has invalid or missing size");
                    if (size == 0 && !hash.equals(func.emptyHash()))
                        throw new IOException(path + " in files array is empty file, but hash isn't the empty hash (" + hash + " != " + func.emptyHash() + ")");
        ...
        if (theirVersion.code() > ourVersion.code()) {
            ...
            for (int i = 0; i < updates; i++) {
                ...
                for (Object o : ver.getArray("changes")) {
                    if (!(o instanceof JsonObject file))
                        throw new IOException("Entry " + o + " in changes array is not an object");
                    var path = file.getString("path");
                    if (path == null) throw new IOException("Entry in changes array is missing path");
                    var fromHash = file.getString("from_hash");
                    if (fromHash != null && fromHash.length() != func.sizeInHexChars())
                        throw new IOException(path + " in changes array from_hash " + fromHash + " is wrong length (" + fromHash.length() + " != " + func.sizeInHexChars() + ")");
                    long fromSize = file.getLong("from_size", -1);
                    if (fromSize < 0)
                        throw new IOException(path + " in changes array has invalid or missing from_size");
                    if (fromSize == 0 && (fromHash != null && !fromHash.equals(func.emptyHash())))
                        throw new IOException(path + " from in changes array is empty file, but hash isn't the empty hash or null (" + fromHash + " != " + func.emptyHash() + ")");
                    var toHash = file.getString("to_hash");
                    if (toHash != null && toHash.length() != func.sizeInHexChars())
                        throw new IOException(path + " in changes array to_hash " + toHash + " is wrong length (" + toHash.length() + " != " + func.sizeInHexChars() + ")");
                    long toSize = file.getLong("to_size", -1);
                    if (toSize < 0) throw new IOException(path + " in changes array has invalid or missing toSize");
                    if (toSize == 0 && (toHash != null && !toHash.equals(func.emptyHash())))
                        throw new IOException(path + " to in changes array is empty file, but hash isn't the empty hash or null (" + toHash + " != " + func.emptyHash() + ")");
            ...
            if (!needsUrlLookup.isEmpty()) {
                ...
                for (var e : needsUrlLookup)
                    byPathAndHash.put(new PathAndHash(e.getKey(), e.getValue().state.hash()), e.getValue());
                for (int code = ourVersion.code(); code >= 1 && !byPathAndHash.isEmpty(); code--) {
                ...
                // Also check bootstrap manifests for remaining objects without direct links
                if (!byPathAndHash.isEmpty()) {
                    Log.debug("Checking bootstrap.json for explicit URLs for " + byPathAndHash.size() + " remaining file(s)");
                    try {
                        JsonObject bootstrap = RequestHelper.loadJson(src.resolve("bootstrap.json"), 16 * M, src.resolve("bootstrap.sig"));
                        checkManifestFlavor(bootstrap, "bootstrap", it -> it == 1);
                        for (Object o : bootstrap.getArray("files")) {
                            if (byPathAndHash.isEmpty()) break;
                            if (!(o instanceof JsonObject file)) continue;
                            var path = file.getString("path");
                            var hash = file.getString("hash");
                            if (path == null || hash == null) continue;
                            var to = byPathAndHash.get(new PathAndHash(path, hash));
                            if (to == null) continue;
                            String urlStr = RequestHelper.checkSchemeMismatch(src, file.getString("url"));
                            String mirrorUrlStr = RequestHelper.checkSchemeMismatch(src, file.getString("mirror_url"));
                            if (urlStr != null) {
                                Log.debug("Found explicit URL for " + path + " (hash " + hash + ") in bootstrap.json: " + urlStr);
                                to.url = new URI(urlStr);
                                if (mirrorUrlStr != null) {
                                    to.mirrorUrl = new URI(mirrorUrlStr);
                                }
                                byPathAndHash.remove(new PathAndHash(path, hash));
                            }
                        }
                    } catch (FileNotFoundException e) {
                        Log.debug("bootstrap.json not found, cannot check for bootstrap URLs");
                    }
                }
                if (!byPathAndHash.isEmpty()) {
                    Log.debug(byPathAndHash.size() + " file(s) have no explicit URL in any prior version or bootstrap; will rely on blob fallback URL");
                }
            }
    ...
    private static int checkManifestFlavor(JsonObject manifest, String flavor, IntPredicate versionPredicate) throws IOException {
        ...
        if (!(flavor.equals(lhs)))
            throw new IOException("Manifest is of flavor " + lhs + ", but we expected " + flavor);
        ...
        if (!versionPredicate.test(rhsI))
            throw new IOException("Don't know how to parse " + str + " manifest (version too new)");
        return rhsI;
    }
