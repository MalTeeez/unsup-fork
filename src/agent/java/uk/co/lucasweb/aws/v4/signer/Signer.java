...
public class Signer {
	...
	String getStringToSign() {
		MessageDigest md = HashFunction.SHA2_256.createMessageDigest();
		byte[] bytes = getCanonicalRequest().getBytes(UTF_8);
		String hashedCanonicalRequest = Bases.bytesToHex(md.digest(bytes));
		return buildStringToSign(date, scope.get(), hashedCanonicalRequest);
	}

	public String getSignature() {
		String signature = buildSignature(awsCredentials.secretKey(), scope, getStringToSign());
		return buildAuthHeader(awsCredentials.accessKey(), scope.get(), request.getHeaders().getNames(), signature);
	}
	...
	private static String buildSignature(String secretKey, CredentialScope scope, String stringToSign) {
		...
		byte[] kDate = hmacSha256(kSecret, scope.dateWithoutTimestamp());
		...
		return Bases.bytesToHex(hmacSha256(kSigning, stringToSign)).toLowerCase();
	}
	...
	public static class Builder {
		...
		public Signer build(Request request, String service, String contentSha256) {
			CanonicalHeaders canonicalHeaders = getCanonicalHeaders();
			String date = canonicalHeaders.getFirstValue(X_AMZ_DATE)
					.orElseThrow(() -> new RuntimeException("headers missing '" + X_AMZ_DATE + "' header"));
		...
		private CanonicalHeaders getCanonicalHeaders() {
			...
			headersList.forEach(h -> builder.add(h.name(), h.value()));
