...
apibase='https://git.sleeping.town/api/v1/repos/exa/unsup'
...
resp=$(curl -s -X POST -H 'Content-Type: application/json' -H "$auth" $apibase/releases --data-raw "
{
	...
	\"draft\": true,
	\"prerelease\": true,
	\"body\": \"Autogenerated snapshot release. See [the commit log](https://git.sleeping.town/exa/unsup/compare/$lasttag...trunk) for changes.\\n\\n***This contains the latest changes, fresh off the trunk. It may be unstable and/or completely broken.***\\n\\n**Note**: Snapshots are signed with a different key from releases. See [the 'keys' directory](https://git.sleeping.town/exa/unsup/src/branch/trunk/keys) in the repo.\"
}")
